...
>
> Grounding: código leído para este documento — `backend/src/api/document.routes.ts`, `frontend/index.html`, `backend/src/documents/sprint/config.ts`, `backend/src/documents/epica/config.ts`, `backend/src/core/generators/pdf.generator.ts`, `backend/src/core/ai/extractor.service.ts`, `docs/planning/PLAN-N8N-SPRINT-WORKFLOW.md`, `docs/planning/AUDITORIA-PLAN-N8N-SPRINT-WORKFLOW.md`.
...
**Cambios del 2026-07-22 (misma sesión, cambios grandes):**
- **Procesa TODAS las filas pendientes del Sheet, no solo la última.** `Normalizar y Validar Fila del Sprint` ahora devuelve un item por cada fila con `Procesado` distinto de `TRUE/SI/YES`, y un nuevo nodo `Loop Over Items - Filas Pendientes` (`splitInBatches`) procesa esas filas una por una de forma secuencial (todo el resto del workflow, desde el cálculo de horas hasta el correo de informe, corre una vez por fila). Antes, si había varias filas en `FALSE`, solo se generaba la última.
- **Bug de schema corregido en la extracción de IA:** `POST /api/sprint/extraer` empezó a fallar con `INTERNAL_ERROR` genérico porque `zodResponseFormat` producía un JSON Schema inválido (un sub-schema de horas reusado por identidad + un campo `.optional()` sin `.nullable()` generaba un `$ref` circular que OpenAI rechazaba). Se corrigió con un patrón de factory function (`crearHorasSegmentoSchema()`, instancia nueva en cada uso) + `.nullable().optional()` en todos los campos opcionales del schema, aplicado a `SprintSchema` y preventivamente a `EpicaSchema`.
- **Reorganización de carpetas en Drive:** `01_SPRINTS` dejó de ser plana — ahora agrupa las subcarpetas de sprint dentro de una subcarpeta por período (`PERIODO JUNIO-JULIO 2026`, `PERIODO JULIO-AGOSTO 2026`, etc.), un nivel más. El workflow ahora busca/crea primero la carpeta de período (mismo nombre que el `Ciclo` sin el prefijo `SPRINT N `, con el prefijo `PERIODO ` en su lugar) y luego busca/crea la subcarpeta del sprint dentro de esa, en vez de directo dentro de `01_SPRINTS`.
- **Festivos y fecha de sprint planning automatizados** — ya no son columnas manuales del Sheet. `Buscar Ciclo en Linear` se movió más temprano en la cadena (corre justo después de leer la fila) para tener `startsAt`/`endsAt` del ciclo cuanto antes; con eso, un nuevo par de nodos (`Obtener Festivos CO`, API pública `date.nager.at` de feriados de Colombia — mismo patrón que usa el Flujo 5 de épica — + `Calcular Dia de Planning y Festivos`) recorre lunes-viernes desde `startsAt` para contar cuántos de esos 5 días son feriados (alimenta la misma fórmula `diasTrabajados = 5 - festivos` de antes) y encontrar el primer día hábil no feriado (la fecha real de planning, reemplaza lo que antes tecleaba el operador).
- **Horas de Reuniones automatizadas desde 3 Google Calendars + notas reales de Gemini.** La columna manual "Horas Reuniones" del Sheet queda opcional: si se deja vacía, se calcula sola; si se llena, ese valor manual gana (mismo patrón para "Horas Personalizaciones", ver abajo). El cálculo: `Listar Reuniones Meet - Daniel/Mauro/Lucho` (un nodo de Google Calendar por persona, credenciales `googleCalendarOAuth2Api` propias de cada uno) traen los eventos con Google Meet de la semana del sprint; se agrupan por reunión distinta (mismo título + mismo inicio — una reunión compartida por los 3 es 1 sola búsqueda de nota, no 3) y se intenta encontrar la nota de reunión de Gemini correspondiente en Drive (carpeta `1CMDwbThDpqLrC-aIOuiHrDi5A_o1abCq`, organizada en 4 subcarpetas por tipo: Dailies, Sesión de Resolución, Sprint Planning, Épica Planning) para sacar la **duración real** de la llamada (línea `"La transcripción finalizó después de HH:MM:SS"` dentro de la nota) en vez de la duración agendada del evento. El match contra esas notas es por la fecha (`YYYY-MM-DD`) al inicio del nombre del archivo, no por el título — los archivos de esas carpetas están renombrados/organizados con ese esquema, no con el título original que genera Gemini (mismo contenido interno, solo el nombre cambia). Si no se encuentra nota para una reunión, cae a la duración agendada. El total se suma en **person-hours**: si los 3 asisten a la misma reunión de 1h, son 3h, no 1h (consistente con que el total de horas del equipo ya es person-hours: `3 personas × 8h × días`). Como estas 7 fuentes (3 calendarios + 4 búsquedas de notas) corren en paralelo, un nodo `Merge` (modo `append`, 7 entradas) las sincroniza antes de procesarlas — conectar varias ramas paralelas al mismo nodo sin un `Merge` explícito no garantiza que n8n espere a todas antes de correr.
- **Personalizaciones**: nueva columna "Horas Personalizaciones" en el Sheet, mismo patrón vacío→automático/lleno→manual. El default es `23.8h/semana` (= 95.25h del mes — Daniel según su cronograma real + Mauro 30% de su capacidad mensual de 160h + Lucho 0h — dividido entre 4 semanas, mismo total ya usado en `HORAS_FIJAS` de épica, ver `backend/src/constants.ts`). Esto reemplazó las columnas genéricas "Segmento adicional - nombre/horas" que ya no existen en el workflow.
- **Zona horaria** ya no es una columna del Sheet: siempre es `America/Bogota`, hardcodeado en el nodo que calcula el corte de planning.
- **`riesgoTransversalResultado` (resumen-v2): dos bugs de confiabilidad del LLM corregidos.** (1) El campo salía `null` de forma recurrente (no solo una vez) en documentos que sí calificaban, porque el criterio de inclusión le pedía al modelo juzgar por su cuenta "¿este documento describe resultados reales?" — se cambió a un criterio directo atado al campo `tiempoVerbal` (que el modelo ya extrae de forma confiable en otros campos del mismo documento), en vez de pedirle un juicio nuevo y ambiguo. (2) Cuando el campo sí se llenaba, las cifras que citaba (issues planeados/agregados/completados) podían venir mal contadas por el LLM sobre su propia lista de issues (ej. "20 de los 20" cuando en realidad eran 15). Se corrigió quitándole al LLM la tarea de contar del todo: ahora solo redacta la frase cualitativa sobre si entraron incidencias que consumieran el colchón reservado, y `componerDatosSprint()` calcula las cifras reales a partir de los issues ya extraídos y arma la frase numérica de forma determinística (con singular/plural correcto y "completó todos los N" cuando se completó el 100%), concatenándola después de la frase del LLM. Ver Caso borde C (Flujo 4) para el patrón general.
...
| Linear (API GraphQL) | Fuente de verdad de los ciclos, issues y su historial — consultada con 3 llamadas HTTP directas (`Buscar Ciclo en Linear`, `Listar Issues del Ciclo`, `Obtener Historial de Issue` una vez por issue), sin ningún LLM de por medio. También aporta `startsAt`/`endsAt` del ciclo, usados para calcular festivos/fecha de planning y la ventana de búsqueda de reuniones |
| Nager.Date (API pública de feriados) | Fuente de los feriados de Colombia (`date.nager.at`) usados para calcular cuántos días de la semana del sprint son festivos y cuál fue el primer día hábil real de planning — mismo patrón que ya usaba el Flujo 5 de épica |
| Google Calendar (3 cuentas: Daniel, Mauro, Lucho) | Fuente de las reuniones reales de la semana del sprint (eventos con Google Meet) para calcular `Horas Reuniones` automáticamente — requiere que Mauro y Lucho compartan su calendario con la cuenta que usan las credenciales de n8n |
...
| Google Sheets ("Preguntas Skill") | Guarda la configuración de cada corrida (ciclo, plantilla, horas reales de Incidencias y, opcionalmente, Reuniones/Personalizaciones si se quiere sobreescribir el cálculo automático) y el resultado (link de Drive, timestamp, `Procesado`) — festivos, fecha de planning y zona horaria ya no son columnas del Sheet |
...
2. n8n toma **todas** las filas pendientes (no procesadas) de la Sheet de configuración de sprints y las procesa una por una, de punta a punta cada una, en orden.
...
4. n8n busca el ciclo en Linear por nombre exacto para obtener sus fechas (`startsAt`/`endsAt`), calcula cuántos feriados de Colombia caen en la semana del sprint y la fecha real del primer día hábil (el "sprint planning" real, aunque haya caído en feriado y se haya corrido de día).
5. n8n calcula de forma determinística (sin LLM) las horas del equipo para ese sprint (Reuniones desde los 3 calendarios + notas de Gemini, Personalizaciones con su default, Incidencias del Sheet, Proyectos como el resto) y el corte de fecha que separa "planeado" de "agregado".
...
7. n8n clasifica cada issue de forma determinística (`agregado`, `type`, `priority`, `status` — comparación de fechas y mapeo de valores, sin LLM) y consolida todo en un único Markdown estructurado, junto con `porcentajeCompletado`/`estadoSprint` ya calculados.
8. n8n envía ese Markdown a `POST /api/sprint/extraer` (el mismo endpoint que usa el flujo manual) para que redacte el texto narrativo (`objetivo` por miembro, `equipo`, `riesgoTransversal`, `desviaciones`, la parte cualitativa de `riesgoTransversalResultado`) respetando los rangos de caracteres del prompt.
9. n8n **pisa** en la respuesta del LLM los campos ya calculados de forma determinística (`sprintName` sin la palabra "SPRINT" — las plantillas HTML ya la traen escrita, para que no quede duplicada —, fechas, `weekNumber`, `estadoSprint`, `porcentajeCompletado`, `horas`, `plantilla`, y las cifras numéricas de `riesgoTransversalResultado`) para que nunca dependan de lo que el LLM haya podido inferir o contar. **No hay validación de rangos de caracteres ni reintento** (existieron hasta el pilot inicial; se eliminaron después — ver ADR-0006): si el texto narrativo queda fuera del rango que pide `SPRINT_SYSTEM_PROMPT`, el PDF se genera igual.
10. n8n llama al mismo endpoint que usa el frontend manual (`POST /api/sprint/pdf`), autenticado con una API key propia del backend.
11. n8n confirma que la respuesta sea realmente un PDF (y no un JSON de error) antes de subir nada a Drive.
12. n8n calcula el nombre del archivo según la plantilla (`RESUMEN_INICIO_...`, `RESUMEN_FINAL_...`, `DETAIL_...`, ver Postcondiciones), busca/crea primero la carpeta del período y luego la subcarpeta del sprint dentro de ella, revisa si ya existe un archivo con ese nombre y le agrega un sufijo autoincremental (`_1`, `_2`, ...) si hace falta, antes de subirlo a Google Drive.
13. n8n relee el Sheet en el instante y ubica en código el `row_number` real de la fila de origen (ver arriba), y la marca como procesada con ese `row_number`, el link del archivo y la fecha.
...
- El PDF queda disponible en la carpeta de Drive `01_SPRINTS`, dentro de una subcarpeta por período (`PERIODO <MES-MES AÑO>`, buscada por nombre exacto; creada si no existe) y, dentro de esa, una subcarpeta por sprint (mismo criterio buscar/crear), con un nombre determinístico según la plantilla usada: `RESUMEN_INICIO_SPRINT_<N>_<MES-MES_AÑO>.PDF` (`resumen-inicio`), `RESUMEN_FINAL_SPRINT_<N>_<MES-MES_AÑO>.PDF` (`resumen`/`resumen-v2`) o `DETAIL_SPRINT_<N>_<MES-MES_AÑO>.PDF` (`detail`), todo en mayúsculas; si ya existe un archivo con ese nombre, se le agrega `_1`, `_2`, etc.
...
No se repiten aquí en detalle. El diseño original y su auditoría (con hallazgos ya resueltos en la construcción real, como la rama IF que distingue un PDF válido de un JSON de error antes de subir a Drive) viven en `docs/planning/PLAN-N8N-SPRINT-WORKFLOW.md` y `docs/planning/AUDITORIA-PLAN-N8N-SPRINT-WORKFLOW.md`; el cambio de arquitectura (AI Agent → extracción determinística) y sus consecuencias viven en `docs/adr/0006-extraccion-deterministica-en-vez-de-ai-agent-para-sync-de-linear.md`. Pendiente:
...
- **KPI de horas adicional en `resumen-v2` — implementado en backend (2026-07-16), pendiente de wiring en n8n.** En el Sprint Planning 4 (2026-07-14, notas en `docs/planning/2026-07-14_SprintPlanning_Sprint4_JunioJulio.md`) Edgar señaló que el KPI "global" (issues Done / issues planeados, conteo puro) no refleja el sobreesfuerzo real cuando se trabajan muchas más horas de las planeadas (ejemplo real: 78-80h planeadas vs. 116h trabajadas dieron 109% cuando él esperaba ~130%). Se descartó reemplazar el "global" por uno basado solo en horas (pierde la señal de si el trabajo realmente se completó) y también excluir issues `Cancelled` del denominador (un issue cancelado sigue siendo evidencia de mala planeación). El "global" de issues se deja exactamente como está; se agregó un KPI de horas **adicional** (4to bloque del header de `resumen-v2`, mismos rangos de color que "global" vía `resolverEstadoGlobal`): `SprintSchema` gana un campo opcional `horas.segmentos[].horasPlaneadas` (solo aplica al segmento "Proyectos (3 objetivos)"; la IA nunca lo completa, ver `SPRINT_SYSTEM_PROMPT`) y `componerDatosSprint()` calcula `horasPorcentaje = round(horas/horasPlaneadas * 100)` sobre ese segmento — el KPI (`horasKpiDisponible`) solo aparece si el campo viene informado. **Pendiente:** n8n (`rqkqaSiaFq0eK7lU`) todavía no rellena `horasPlaneadas` al armar el payload del documento de cierre — hay que agregar el paso que busca la fila `resumen-inicio` del mismo Ciclo+weekNumber y copia (o recalcula desde su `Festivos`) el valor planeado de Proyectos antes de `Consolidar Payload Final del Sprint`. Sin ese wiring, el KPI de horas simplemente no aparece (comportamiento seguro, no rompe nada).
...
## Referencias
- Plan técnico original de la automatización (AI Agent, registro histórico): `docs/planning/PLAN-N8N-SPRINT-WORKFLOW.md`
