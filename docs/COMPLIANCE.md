...
|---|---|---|
| Protección de datos personales (Ley 1581 Colombia / GDPR / LGPD) | **Posiblemente, en grado mínimo — pendiente de confirmar** | El sistema almacena y muestra nombres de personas del equipo (`members[].name` en `SprintSchema`, ej. "Luis", "Mauricio", "Daniel") dentro de reportes internos de sprint. Es un dato personal (nombre) pero de empleados internos en un reporte interno de desempeño, no de clientes/terceros — el análisis de qué régimen aplica (si alguno, más allá de política interna de RRHH) requiere confirmación legal, no es algo que este documento pueda resolver. **Ampliación 2026-07-22:** el workflow de n8n del Flujo 3 (`rqkqaSiaFq0eK7lU`) ahora también lee los Google Calendars personales de los 3 miembros del equipo (eventos, asistentes, horarios) y, cuando existe, descarga y lee el contenido completo de notas de reunión generadas por Gemini — que incluyen la **transcripción textual** de lo hablado en la reunión, con nombre de cada persona junto a sus intervenciones — para extraer la duración real de la llamada. Esto es un procesamiento de datos personales bastante más sensible que solo mostrar nombres en un reporte (contenido de conversaciones internas, no solo metadata), y no estaba contemplado en la evaluación original de este documento. Mismo punto abierto: requiere confirmación legal/organizacional, no es algo que este análisis técnico pueda resolver por sí solo. |
...
Ninguna de las áreas evaluadas indica una obligación regulatoria clara y urgente para el estado actual del proyecto (herramienta interna, sin base de datos, sin clientes externos, sin datos financieros/de salud). El punto que más vale la pena que el equipo confirme formalmente es el tratamiento de datos del equipo en los reportes de sprint — no porque haya evidencia de riesgo, sino porque la guía interna pide no asumir "no aplica" sin hacer la evaluación explícita, y esa confirmación final es de alcance legal/organizacional, no técnico. Ese punto se volvió más relevante el 2026-07-22: ya no es solo el nombre de cada persona en un reporte, sino que el workflow de n8n lee calendarios personales y transcripciones de reuniones (vía notas de Gemini) para calcular horas — vale la pena que esta confirmación explícita cubra también ese flujo de datos, no solo los nombres en `SprintSchema`.
