...
### Plantilla `resumen-v2` de `sprint`

Versión deliberadamente más simple que `resumen`: mismo header/tarjetas por miembro, pero el bloque de KPIs del header queda recortado a solo 2 (`planPorcentajeCompletado`/`planEstadoSprint` y `agregadoPorcentajeCompletado`/`agregadoEstadoSprint`, ya calculados en `componerDatosSprint()` para `resumen-v3` — `resumen-v2` no calcula nada nuevo, solo deja de renderizar `globalPorcentaje` y el KPI de horas), sin el box de horas por miembro ni las tarjetas de "Desviaciones de alcance" que sí tiene `resumen-v3`. `SprintSchema` gana un campo opcional a nivel de documento, `riesgoTransversalResultado` (string, máx. 260 caracteres) — mismo patrón que el de `epica` (ver arriba): si el riesgo transversal previsto en `resumen-inicio` se materializó o no, y qué pasó en la práctica. Solo `template-resumen-v2.html` lo renderiza (bajo el texto de `riesgoTransversal` en el mismo box, con un divisor y label "RESULTADO", igual estructura visual que `template-cierre.html` de `epica`); el resto de plantillas lo ignoran aunque venga informado. `SPRINT_SYSTEM_PROMPT` instruye a la IA a omitirlo por completo si el documento fuente es solo un plan sin resultados reales, igual criterio que `desviaciones`/`cumplimiento`.

El propio `riesgoTransversal.texto/mitigacion` (usado también por `resumen-inicio`) está acotado a un único tema: el riesgo de que aparezcan incidencias no planeadas que consuman las horas reservadas para el segmento "Incidencias" del bloque `horas` — ya no es un riesgo genérico inferido del documento. La idea es que el riesgo previsto al inicio y su resultado al cierre (`riesgoTransversalResultado`) cuenten la misma historia: los issues se planearon reservando ese colchón de horas: si no entran incidencias, el sprint cierra según lo planeado (o incluso hay margen para sumar agregados); si entran, consumen el colchón antes que afectar lo planeado.

Todo texto narrativo de un documento de **resumen fin** (`resumen`, `resumen-v2`, `resumen-v3` — cualquier extracción que describa resultados reales, no un plan) debe quedar redactado en pasado, `riesgoTransversal.texto/mitigacion` incluido (ej. "el riesgo era que aparecieran incidencias..." en vez de "el riesgo es que aparezcan..."). Esto lo gobierna el campo `tiempoVerbal` (Futuro/Pasado) del documento fuente, que ya venía de antes (ver `SPRINT_SYSTEM_PROMPT`) y no es parte del `SprintSchema` de salida — un `resumen fin` siempre debe extraerse con `tiempoVerbal: "Pasado"`; `resumen-inicio` usa `"Futuro"`. Como la extracción es la misma llamada a `/extraer` sin importar la plantilla, esto implica que inicio y cierre de un mismo sprint parten de dos documentos fuente (y dos extracciones) distintas, no una sola reutilizada para ambos.
